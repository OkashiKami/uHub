...
//			Undo.RecordObject (script, "MapMagic settings change");
            ...
            layout.Par(8); layout.Foldout(ref script.guiWaterSettings, "Water Settings");
            if(script.guiWaterSettings)
            {
                layout.Field(ref script.WaterPrefab, "Prefab");
                layout.Field(ref script.useMainCamera, "Follow Main Camera");
                if (!script.useMainCamera)
                {
                    layout.Field(ref script.target, "Target");
                }
                else layout.Label("\t\tFollowing Main Camera");
                layout.Field(ref script.WaterLevl, "Water Level", min: 0, max: 30, slider: true);
            }
